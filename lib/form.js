...
module.exports = function() {
  ...
  this.Given(/^I fill in "([^"]*)" with "([^"]*)"$/, function(arg1, arg2, callback) {
    var action = function(_by) {
      return driver.findElements(_by).then(function(inputs) {
    ...
    if (arg1.match(/^#.*/)) { // find by id if hash is found
      action(by.id(arg1.replace('#', ''))).then(callback);
    } else if (arg1.match(/^\..*/)) { // find by css if . is found
      action(by.css(arg1)).then(callback);
      ...
      action(by.xpath('//*[@name="' + arg1 + '"]')).then(callback);
    }
  });
