...
module.exports = function() {
  ...
  this.Given(/^I fill in "([^"]*)" with "([^"]*)"$/, function(arg1, arg2, callback) {
    ...
    self.browser.findElement(self.By.xpath('//*[name="' + arg1 + '"]')).then(function(input) {
      action(input);
    }).addErrback(function(e) {
      self.browser.findElement(self.By.xpath('//label[text()="' + arg1 + '"]')).then(function(label) {
        label.getAttribute('for').then(function(id) {
          self.browser.findElement(self.By.id(id)).then(function(input) {
            action(input);
          });
        });
      }).addErrback(function(e) {
