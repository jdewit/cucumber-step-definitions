...
module.exports = function() {
  ...
  this.Given(/^I press "([^"]*)"$/, function(arg1, callback) {
    if (arg1.match(/#.*/)) {
      ...
      driver.findElements(by.xpath('//button[text()="' + arg1 + '"]')).then(function(elems) {
        return elems[elems.length - 1]; // click the last save button in case there are multiple forms
      }).then(function(el) {
        return el.click();
      }).then(function() {
        callback();
  ...
  this.Given(/^I click on "([^"]*)"$/, function(arg1, callback) {
    ...
    var handleError = function(e) {
      if (attempts < 5) {
        attempts += 1;
        step(attempts).then(callback, handleError);
      } else {
        callback.fail(e.message);
      }
    };
    var step = function(attempts) {
      if (arg1.match(/^#.*/)) { // find by id
        return driver.findElement(by.id(arg1.replace('#', ''))).then(function(el) {
          return el.click();
        });
      } else if (arg1.match(/^\..*/)) { // find by class
        return driver.findElements(by.css(arg1)).then(function(els) {
          return els[els.length - 1].click();
        });
      } else { // find by link text
        return driver.findElements(by.linkText(arg1)).then(function(els) {
          return els[els.length - 1].click();
        });
      }
    };
    step(0).then(callback, handleError);
  });
