...
module.exports = function() {
  ...
  this.Then(/^I should see "(.*)"$/, function(arg1, callback) {
    driver.findElement(by.xpath('//div[text()="' + arg1 + '"]')).then(function(el) {
      callback();
  ...
  this.Then(/^I should not see "(.*)"$/, function(arg1, callback) {
    driver.findElements(by.xpath('//*[text()="' + arg1 + '"]')).then(function(el) {
      if (el.length === 0) {
        callback();
      } else {
        callback.fail('"' + arg1 + '" should not be found');
      }
    }).addErrback(function(e) {
      callback();
    });
  ...
  this.Then(/^I should see the alert "([^"]*)"$/, function(arg1, callback) {
    ...
    }, 10000, 'Taking too long to find alert').then(function() {
        ...
        elems.forEach(function(el) {
          el.getText().then(function(text) {
            if (text === arg1 || text.replace('×\n', '') === arg1) {
              driver.executeScript('$(".alert").remove();').then(function() { // clean up alert
                callback();
              });
            } else if (el === last) {
  ...
  this.Then(/^"([^"]*)" should have text "([^"]*)"$/, function(arg1, arg2, callback) {
    driver.wait(function(){
      return driver.executeScript('return !!$("' + arg1 + '").length;');
    }, 5000, 'Taking too long to find element').then(function() {
